    ...
    >
      ...
      <div className="relative z-30 flex flex-col justify-between h-full p-6 lg:p-8 w-[60%] pointer-events-none">
        ...
        <div>
          ...
          <h4 
            className="text-[11px] lg:text-[12px] font-bold uppercase tracking-[0.15em] mb-1.5"
            style={{ color: categoryColor }}
          >
            {category}
          </h4>
          
          <h3 className="text-2xl lg:text-[28px] font-bold text-white mb-2 tracking-tight">
            {title}
          </h3>
          
          <p className="text-[15px] md:text-[16px] text-[#a1a1aa] leading-relaxed line-clamp-4 pr-4">
            {description}
          </p>
        </div>

        {/* Tags and Action Link (Bottom Section) */}
        <div className="mt-auto pt-6 pointer-events-auto">
          {/* Tags */}
          <div className="flex flex-wrap items-center gap-1.5 text-[12px] text-[#71717a] mb-5 font-medium">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-0.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
            2025
            <span className="mx-1 text-[#3f3f46]">•</span>
            {tags.map((tag, i) => (
              <React.Fragment key={tag}>
                <span>{tag}</span>
                {i < tags.length - 1 && <span className="mx-1 text-[#3f3f46]">•</span>}
              </React.Fragment>
            ))}
          </div>
          ...
          {/* Action Link */}
          <div 
            className="flex items-center gap-1.5 text-[14px] md:text-[15px] font-bold group/link cursor-pointer w-fit"
            style={{ color: categoryColor }}
          >
            View Case Study 
            <svg 
